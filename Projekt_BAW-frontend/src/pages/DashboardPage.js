...
function Dashboard() {
	...
	const [header, setHeader] = useState(['id', 'brand', 'model', 'gearbox', 'type', 'price', 'availability'])
	...
	const handleData = type => {
		if (type == 'Users') {
			...
			axios.get('http://127.0.0.1:8000/api/cars').then(response => {
				setHeader(['id', 'brand', 'model', 'gearbox', 'fuel_type', 'price', 'available'])
				setData(response.data.data)
				setType('cars')
			})
		} else if (type == 'Rents') {
			...
			<>
												...
												{searchResults && searchResults.length > 0
													? searchResults.map(item => (
															<Tr key={item.id}>
																{header.map(column => {
																	if (column == 'available')
																		return <Td key={item.id}>{item[column] == 0 ? 'yes' : 'no'}</Td>
				...
				{isOpen && (
					<Drawer isOpen={isOpen} placement='left' onClose={onClose} size='xs'>
						<DrawerOverlay />
						<DrawerContent>
							<SidebarContent handleData={handleData} w='full' borderRight='none' />
						</DrawerContent>
					</Drawer>
				)}
			</>
