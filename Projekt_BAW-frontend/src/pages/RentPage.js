...
function Rent() {
	...
	useEffect(() => {
		axios.get(`http://127.0.0.1:8000/api/cars/${params.id}`).then(response => {
	...
	function rentACar(e) {
		...
		if (rental_date < now || return_date < now) {
			console.log('Proszę wybrać poprawną datę wypożyczenia i zwrotu')
		} else if (rentDuration <= 0) {
			console.log('Możesz wynająć przynajmniej na 1 dzień.')
		} else {
			const price = (rentDuration / (1000 * 60 * 60 * 24)) * car.price
			...
			if (rentalDate.current.value != '' && returnDate.current.value != '') {
				axios
					...
					.then(response => {
						showToast(toast, 'Wypożyczono!', 'success', 'Success')
						navigate('/cars')
					})
					.catch(error => {
						showToast(toast, 'Wypożyczenie nie powiodło się', 'error', 'Error')
	...
	return (
		<>
			<Navbar sidebarContent={<HomeSidebarContent />} links={<NavbarLinks />} buttons={<AvatarMenu />} />
				...
				>
					...
					<Box w={{ base: '100%', lg: '50%' }} p={'5%'} bg={'white'} h={'full'}>
						<VStack alignItems={'center'} spacing={'3'}>
							...
							<Input type={'date'} ref={rentalDate} onChange={handleRentalDateChange} />
							<FormLabel fontWeight='600' color='gray.600'>
								{t('profile.returnDate')}
							</FormLabel>
							<Input type={'date'} ref={returnDate} onChange={handleReturnDateChange} />
							...
							<SimpleGrid w={'full'} columns={3} py={3} textAlign='center'>
								...
								<GridItem>
									...
									<Text fontWeight='600' color='gray.600'>
										{car.available === 1 ? t('carCard.yes') : car.available === 0 ? t('carCard.no') : car.available}
									</Text>
								</GridItem>
							</SimpleGrid>
