...
function ProfileDrawer() {
	...
	const handleSubmit = () => {
		axios
			.put(`http://127.0.0.1:8000/api/users/${user_id}`, formData)
			.then(response => {
	...
	return (
		<>
			<Button leftIcon={<EditIcon color={'white'} />} colorScheme='telegram' onClick={onOpen}>
			...
			<Drawer isOpen={isOpen} placement='right' initialFocusRef={firstField} onClose={onClose}>
				...
				<DrawerContent>
					...
					<DrawerHeader borderBottomWidth='1px'>{t('profile.modifyProfile')}</DrawerHeader>
					...
					<DrawerBody>
						<Stack spacing='24px'>
							...
							<Box>
								<FormLabel htmlFor='telephone'>{t('profile.phoneNumber')}</FormLabel>
